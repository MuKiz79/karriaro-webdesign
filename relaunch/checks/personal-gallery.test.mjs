...
test('the curated gallery opens three complete fictional personal sites', async () => {
  ...
  assert.equal(document.querySelectorAll('.pl-archive-item').length, 0);
  ...
  for (const link of featured.map(person => person.querySelector('a[href^="/personen/"]'))) {
    ...
    assert.ok((page.match(/<section\b/g) || []).length >= 5, `${href} must have a full visitor journey`);
    assert.match(page, /class="pc-demo-form"/, `${href} needs a persona-specific contact ending`);
  }
