...
intro = '<section class="page-hero gallery-hero wrap"><p class="eyebrow">Karriaro / Werkschau</p><h1>Jede Aufgabe<br><em>eine eigene Welt.</em></h1><p>Sechs ausgewählte Website-Konzepte und elf weitere Entwürfe zeigen, wie unterschiedlich Gestaltung und Funktion sein können. Alle Seiten lassen sich öffnen und ausprobieren. Dazu kommt mein veröffentlichter persönlicher Auftritt.</p><nav class="gallery-jump" aria-label="Bereiche der Werkschau"><a href="#konzepte">Ausgewählte Websites ↘</a><a href="#neue-konzepte">Drei neue Branchen ↘</a><a href="#sammlung">Alle Entwürfe ↘</a><a href="#veroeffentlicht">Veröffentlichte Website ↘</a></nav></section>'
...
gallery += '</section>'
gallery += '<section class="gallery-new" id="neue-konzepte" aria-labelledby="new-title"><div class="wrap gallery-new-intro"><div><p class="eyebrow">02 / Neue Branchen</p><h2 id="new-title">Ein guter Auftritt<br><em>löst etwas aus.</em></h2></div><p>Drei weitere Websites zeigen konkrete Besucherwege: Terminwahl, Stilfindung und technische Anfrage. Jede hat eine eigene Formensprache.</p></div><div class="wrap gallery-new-grid">'
new_labels = {'raum': ('Gesundheit / Orientierung', 'Ein ruhiger Anfang.', 'Leistungen verstehen und den Terminweg ausprobieren.'), 'halo': ('Salon / Persönlichkeit', 'Stil mit Haltung.', 'Stilwelt wählen und den passenden Besuch planen.'), 'vecto': ('Industrie / Präzision', 'Eine klare Richtung.', 'Bauteilfinder bedienen und eine Projektanfrage vorbereiten.')}
for index, slug in enumerate(new_slugs, start=1):
    _, name, kind, image, _ = next(project for project in projects if project[0] == slug)
    label, headline, description = new_labels[slug]
    gallery += f'<a class="gallery-new-card gallery-new-card--{slug}" href="{project_url(slug)}" aria-label="Website-Konzept {name} öffnen"><span class="gallery-new-media"><img src="/studien/assets/{image}" alt="" width="1536" height="1024" loading="lazy" decoding="async"><span>0{index} / 03</span></span><span class="gallery-new-body"><small>{label}</small><strong>{headline}</strong><span>{description}</span><b>Website öffnen <span aria-hidden="true">↗</span></b></span></a>'
gallery += '</div></section>'
gallery += published

archive_projects = [project for project in projects if project[0] not in featured_slugs + new_slugs]
gallery += f'<section class="gallery-archive wrap" id="sammlung" aria-labelledby="archive-title"><div class="gallery-archive-head"><div><p class="eyebrow">03 / Weitere Websites</p><h2 id="archive-title">Elf weitere<br><em>Perspektiven.</em></h2></div><p>Auch diese Entwürfe sind vollständige, bedienbare Seiten. Wählen Sie ein Feld und sehen Sie selbst, was die Website kann.</p></div><div class="gallery-index">'
