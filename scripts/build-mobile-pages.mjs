...
const MOBILE_OVERRIDES_LINK = `    <link rel="preload" as="style" href="/css/mobile-overrides.css?v=434">
    <link rel="stylesheet" href="/css/mobile-overrides.css?v=434" media="print" onload="this.media='all'">
    <noscript><link rel="stylesheet" href="/css/mobile-overrides.css?v=434"></noscript>
...
const PWA_HEAD_BLOCK = `    <link rel="manifest" href="/manifest.json">
    ...
    <!-- Sprint 166 — Preconnect zu karriaro-webdesign.de (Cross-Origin-Bilder
...
body{margin:0;font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','SF Pro Text','Inter',system-ui,sans-serif;background:#FFFFFF;color:#1D1D1F;-webkit-font-smoothing:antialiased;text-rendering:optimizeLegibility}
main{display:block}
nav{position:fixed;top:0;left:0;right:0;z-index:60;background:rgba(255,255,255,0.92);-webkit-backdrop-filter:saturate(180%) blur(20px);backdrop-filter:saturate(180%) blur(20px);height:64px;display:flex;align-items:center;border-bottom:1px solid rgba(0,0,0,0.04)}
        ...
        <article class="m-demo-swiper-slide" data-m-demo-slide="${i}">
            <button type="button" class="m-demo-swiper-card m-poster" data-m-demo-open data-m-demo-href="${href}" data-m-demo-title="${title}" data-m-demo-domain="${domain}" aria-label="${title} Live-Demo öffnen">
                ...
                <div class="m-poster-stage" data-m-poster-stage>
                    <iframe class="m-poster-frame" src="${href.replace(/\.html$/, '-embed-desktop.html')}" title="${title} Desktop-Hero-Vorschau" width="1280" height="1024" loading="eager" sandbox="allow-same-origin allow-scripts" tabindex="-1" aria-hidden="true" fetchpriority="high"></iframe>
                </div>
            </button>
