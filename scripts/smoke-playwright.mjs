...
const HAUPTSEITE_CASES = [
    ...
    // Sprint 172 — WERKZEUG-Sektion entfernt, Magic-Moment-Sektion (Sprint 168) bleibt.
    // Tests adressieren #kr-audit-magic-url + [data-audit-magic-stage] der neuen Sektion.
    { name: 'erste-einschaetzung · form-submit-renders-result',
        action: async (page) => {
            await page.locator('#kr-audit-magic-url').fill('https://kanzlei-mueller.de');
            await page.locator('[data-audit-magic] [data-audit-magic-submit]').click();
            await sleep(3500);
            const result = await page.locator('[data-audit-magic-stage]').innerText().catch(() => '');
            return { pass: result.length > 50, msg: `result-len=${result.length}` };
        }
    },
    { name: 'erste-einschaetzung · www-url-akzeptiert',
        action: async (page) => {
            await page.locator('#kr-audit-magic-url').fill('www.kablan-immobilien.de');
            await page.locator('[data-audit-magic] [data-audit-magic-submit]').click();
            await sleep(3500);
            const result = await page.locator('[data-audit-magic-stage]').innerText().catch(() => '');
            return { pass: result.length > 50, msg: `result-len=${result.length}` };
    ...
    { name: 'erste-einschaetzung · bare-domain-akzeptiert',
        action: async (page) => {
            await page.locator('#kr-audit-magic-url').fill('kablan-immobilien.de');
            await page.locator('[data-audit-magic] [data-audit-magic-submit]').click();
            await sleep(3500);
            const result = await page.locator('[data-audit-magic-stage]').innerText().catch(() => '');
            return { pass: result.length > 50, msg: `result-len=${result.length}` };
